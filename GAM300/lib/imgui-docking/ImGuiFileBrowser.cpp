...
* \brief	Class and fn definitions for the file browser for InGui system usage
* \author	Shaikh Faid Bin Omar, 100% Code Contributions
*
*
...
namespace imgui_addons
{
    ...
    bool ImGuiFileBrowser::showFileDialog(const std::string& label, const DialogMode mode, const ImVec2& sz_xy, const std::string& valid_types_temp)
    {
        ...
        if (ImGui::BeginPopupModal(label.c_str(), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
        {
            ...
            if(is_appearing)
            {
                ...
                if(mode != DialogMode::SELECT)
                {
                    this->valid_types = valid_types_temp;
                    setValidExtTypes(valid_types_temp);
                }
    ...
    bool ImGuiFileBrowser::readDIR(std::string pathdir)
    {
        ...
        if (dir != nullptr)
        {
            ...
            while ((ent = readdir (dir)) != nullptr)
            {
                ...
                {
                    #ifdef OSWIN
                    std::string dir_temp = pathdir + std::string(ent->d_name);
                    // IF system file skip it...
                    if (FILE_ATTRIBUTE_SYSTEM & GetFileAttributesA(dir_temp.c_str()))
                        continue;
                    if (FILE_ATTRIBUTE_HIDDEN & GetFileAttributesA(dir_temp.c_str()))
                        is_hidden = true;
                    #else
                    if(name[0] == '.')
                        is_hidden = true;
    ...
    void ImGuiFileBrowser::filterFiles(int filter_mode_temp)
    {
        filter_dirty = false;
        if(filter_mode_temp | FilterMode_Dirs)
        {
            filtered_dirs.clear();
        ...
        if(filter_mode_temp | FilterMode_Files)
        {
            filtered_files.clear();
