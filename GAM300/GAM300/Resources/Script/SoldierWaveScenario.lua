...
function resetAll()
	...
	diedTimer = 2
	eEnemyCount = 0
	pEnemyPrefabID = ECS:getPrefabID("Prefab_Soldier")
	...
	if eEnemyTimer <= 0 then
		if spawnEnemyCount < eEnemyCountMax then
			...
			spawnPos.z = pos.z
					...
					ECS:addEntityTag(enemyID,"SoldierWave")
					ePrevXPos = eXPos
					ePrevZPos = eZPos
...
function updateEnemyCount()
	enemies = ECS:getEntityTagged("SoldierWave")
...
function Update(dt)
	if stopScript == false then
			...
			if eEntID == ECS:getEntityID("SoldierWaveScenario") then
				if ECS:hasComponent_Transform(eEntID) then
			...
			if isTriggered == true then
				...
				if ECS:hasComponent_SpriteCom(UIObjID) then
					...
					spriteCom.SpriteName = "challenge_solider"
				end
