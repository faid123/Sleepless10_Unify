...
function Start()
    ...
    player_env["Player_Env"]["currentVideo"] = 3
    Console.WriteLine("boss defeat start called")
    cutsceneMapID = ECS:getPrefabID("Prefab_AAACutSceneMap1")
...
--[[     eEntID = ScriptSystem:getLuaEntityID()
    ...
    if timer >= 3.0 then
        if Inputs.get_Key(InputKey.Z_KEY) or Inputs.get_Controller(InputControllerKey.B_BUTTON) or (RenderSystem:getPlayVideo() == false) then
            if(currState ~= 3) then
                SetStateEnd()
                Console.WriteLine("getPlayerVideo")
                Console.PrintFloat(RenderSystem:getPlayVideo())
                Console.WriteLine("get_Key")
                Console.PrintFloat(Inputs.get_Key(InputKey.Z_KEY))
                Console.PrintFloat(ScriptSystem:getLuaEntityID())
                ECS:instantiatePrefab(cutsceneMapID, "AAACutSceneMap1", Vector3:new(0,0,1),Quat:new(0,0,0,1),false, false, false)
