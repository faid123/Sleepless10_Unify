...
function Start()
    ...
    --player_env["Player_Env"]["currentVideo"] = 6
    cutsceneMapID = ECS:getPrefabID("Prefab_AAACutSceneMap2")
    ...
    if timer >= 3.0 then
        if Inputs.get_Key(InputKey.Z_KEY) or Inputs.get_Controller(InputControllerKey.B_BUTTON) or (RenderSystem:getPlayVideo() == false) then
            if(currState ~= 3) then
                ...
                ECS:instantiatePrefab(cutsceneMapID, "AAACutSceneMap2", Vector3:new(0,0,1),Quat:new(0,0,0,1),false, false, false)
