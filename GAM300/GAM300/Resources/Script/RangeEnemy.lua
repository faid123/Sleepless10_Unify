...
local E_BulletPrefabID
local fireDelayTimer        = 0.0
local fireDelayTime         = 5.0
local canShoot              = true
local playShootAnim         = false
local playShootAnimTimer    = 0.0
local startShooting         = false
local startShootCountdown   = 0.0
local shotFired             = true

local playerLuaScript

-- Army Scenario
local armyScenarioID
...
function Start()
    ...
    E_BulletPrefabID = ECS:getPrefabID("Prefab_EnemyBullet")
    armyScenarioID = ECS:getEntityTagged("ArmyScenario")
...
function Update(dt)
   ...
   -- end
    ...
    handleShootCountdown(dt)
    handleShootAnimTime(dt)
...
function Unify()
    if eUnified == false then
        ...
        for i = 1, #armyScenarioID do
            if ECS:hasEntity(armyScenarioID[i]) then
                if ECS:hasTagInEntity(armyScenarioID[i], "ArmyScenario") then
                    if ECS:getComponent_LuaScriptCom(armyScenarioID[i]):GetEnvironment().armySceneActivated then
                        ECS:getComponent_LuaScriptCom(UIObjID):GetEnvironment().armyUnifyCount = 
                            ECS:getComponent_LuaScriptCom(UIObjID):GetEnvironment().armyUnifyCount + 1
                    end
                end
            end
        end
...
function handleEnemyLifeTime(dt)
    if dying and death <= 0 and eUnified then
...
function fireWeapon()
    if canShoot then
        playShootAnim = true
        shotFired = false
        playShootAnimTimer = 0.5
        startShootCountdown = 0.3
        canShoot = false
...
function handleShootAnimTime(dt)
    if playShootAnimTimer > 0.0 then
        playShootAnimTimer = playShootAnimTimer - dt
    end
...
function handleShootCountdown(dt)
    if playShootAnim then
        if startShootCountdown > 0.0 then
            startShootCountdown = startShootCountdown - dt
        end

        if startShootCountdown <= 0.0 and not shotFired then
            startShooting = true
        end
    end
end
...
function spawnBullets()
    if startShooting then
        eTransform = ECS:getComponent_Transform(eEntID)
        local bulletSpawnPos = (eTransform.pos + forwardPrime * forwardOffest) + rightwardPrime * rightwardOffset
        local bulletRotQuat = PhysicsSystem:lookat(bulletSpawnPos, pPos, unitDir)
        local E_BulletID = ECS:instantiatePrefab(E_BulletPrefabID, "EnemyBullet", bulletSpawnPos, bulletRotQuat, false, false, false)
        startShooting = false
        shotFired = true
    end
end