...
-- RiotShield
local eRiotShieldPrefab
local riotShieldPos
local riotShieldID
local equippedShield    = false

-- Shield Scenario
local shieldScenarioID
...
function Start()
    ...
    eRiotShieldPrefab = ECS:getPrefabID("Prefab_RiotShield")
    shieldScenarioID = ECS:getEntityTagged("ShieldScenario")
...
function Update(dt)
    ...
    eShield["eShield_Env"] = eLuaScript:GetEnvironment()
...
function onHit()
    ...
    --    if eUnified == false and not ECS:hasTagInEntity(mContactReportCallback._detectedBullet, "Blocked") then
    --        eHealth = eHealth - 5
    --        eFlashTimer = 8
	--		bEmitter.emit = true
    --    end
    if dying == false and eUnified == false then
        if eRigidDynamic.getHit and mContactReportCallback.laserHitEnemy and canApplyLaserDamage then
...
function Unify()
    if eUnified == false then
        ...
        for i = 1, #shieldScenarioID do
            if ECS:hasEntity(shieldScenarioID[i]) then
                if ECS:hasTagInEntity(shieldScenarioID[i], "ShieldScenario") then
                    if ECS:getComponent_LuaScriptCom(shieldScenarioID[i]):GetEnvironment().shieldSceneActivated then
                        ECS:getComponent_LuaScriptCom(UIObjID):GetEnvironment().shieldUnifyCount = 
                            ECS:getComponent_LuaScriptCom(UIObjID):GetEnvironment().shieldUnifyCount + 1
                    end
                end
            end
        end
...
function handleEnemyLifeTime(dt)
    if dying and death <= 0 and eUnified then
        ...
        if lifeTime <= 0.0 then
            ...
            ECS:addEntityToDestroyQueue(riotShieldID)
            PhysicsSystem:LuaRemoveActor(riotShieldID)
...
function equipRiotShield()
    riotShieldPos = eRigidDynamic.rigidPos + forwardPrime * forwardOffset
    riotShieldID = ECS:instantiatePrefab(eRiotShieldPrefab, "RiotShield", riotShieldPos,eRotQuat,false, false, false)
    eShield["eShield_Env"]["RiotShieldID"] = riotShieldID
    local rsRigidStatic = ECS:getComponent_RigidStatic(riotShieldID)
    rsRigidStatic:setActorName("RiotShield")
end
