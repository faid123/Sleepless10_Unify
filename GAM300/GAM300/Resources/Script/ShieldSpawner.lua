...
function resetAll()
	...
	eEnemyCount = 0
	setMaxEnemy(5)
	pEnemyPrefabID = ECS:getPrefabID("Prefab_Shield")
end
		...
		if eEnemyCount < eEnemyCountMax then
			...
			-- if ePrevXPos ~= eXPos and ePrevZPos ~= eZPos then
				-- if isEnemyInsight(spawnPos, playerCamRot, 100) == false then
					local enemyID = ECS:instantiatePrefab(pEnemyPrefabID, "ShieldEnemy", spawnPos, eRot, false, false, false)
					-- eEnemyCount = eEnemyCount + 1
					----Console.WriteLine("shield enemy spawned")
					-- --Console.PrintFloat(eEnemyCount)
					ePrevXPos = eXPos
					ePrevZPos = eZPos
				-- end
			-- end
			----Console.WriteLine("ShieldSpawner script running")
		end
		eEnemyTimer = 8
	end
...
function updateEnemyCount()
	enemies = ECS:getEntityTagged("Shield")
...
function Update(dt)
	...
	updateEnemyCount()
    if ECS:hasTagInEntity(eEntID, "ShieldSpawner") == true then
        if ECS:hasComponent_Transform(eEntID) then
