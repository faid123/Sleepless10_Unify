...
function Start()
	...
	lowerBodyEntID = ECS:getEntityID("Boss_LowerBody")
...
function Update(dt)
	...
	if currWanted >= 5 and ECS:getComponent_LuaScriptCom(UIObjID):GetEnvironment().allObjectivesCleared then
		if  ECS:hasComponent_Transform(EntID1) then
	...
	if bossHP < 95  then
		...
		if turnBool and coolBool then
			if canPlayLaserSound and bossFightStarted then
