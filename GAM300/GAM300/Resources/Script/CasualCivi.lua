...
-- If trapped Variables
local invincible        = false

-- Civilian Scenario
local civiScenarioID
...
function Start()
    civiScenarioID = ECS:getEntityTagged("CivilianScenario")
...
function Unify()
    if eUnified == false then
        ...
        for i = 1, #civiScenarioID do
            if ECS:hasEntity(civiScenarioID[i]) then
                if ECS:hasTagInEntity(civiScenarioID[i], "CivilianScenario") then
                    if ECS:getComponent_LuaScriptCom(civiScenarioID[i]):GetEnvironment().civiSceneActivated then
        ...
        eMaterial.m_model = "HappyCasualCiviBone"
        eMaterial.m_texture = "HappyCasualCivi"
