		...
		if eEnemyCount < eEnemyCountMax then
			...
			-- if ePrevXPos ~= eXPos and ePrevZPos ~= eZPos then
				-- if isEnemyInsight(spawnPos, playerCamRot, 100) == false then
					local enemyID = ECS:instantiatePrefab(pEnemyPrefabID, "ArmyEnemy", spawnPos, eRot, false, false, false)
					-- eEnemyCount = eEnemyCount + 1
					------Console.WriteLine("amy enemy spawned")
...
function Update(dt)
	...
	updateEnemyCount()
    if ECS:hasTagInEntity(eEntID, "ArmySpawner") == true then
        if ECS:hasComponent_Transform(eEntID) then
