...
local g_lowerBodyScript = {}
local lowerBodyEntID
local tagList
local foundLowerBody = false
local cwBool            = false
...
function Start()
    ...
    --EntID1 = ECS:getEntityID("Boss_UpperBody")
    -- Console.PrintEntityID(EntID1)
	--playerID = ECS:getEntityID("Player")
	-- lowerBodyEntID = ECS:getEntityID("3_BossLowerTurr")
...
function Update(dt)
	tagList = ECS:getEntityTagged("3bu") 
	...
	if foundLowerBody == false then
		tagList = ECS:getEntityTagged("LowerLaser")
		for i=0, #tagList, 1 do
			if tagList[i] ~= 0 then
				lowerBodyEntID = tagList[i]
				foundLowerBody = true
				Console.WriteLine("LOWER BODY ID FOUND")
				Console.WriteLine(lowerBodyEntID)
			end
		end
	end
	...
	if ECS:hasComponent_LuaScriptCom(lowerBodyEntID) then
        g_lowerBodyScript = ECS:getComponent_LuaScriptCom(lowerBodyEntID)
    end
	
	
	cwBool     = g_lowerBodyScript:GetEnvironment().cwBool
	turnBool   = g_lowerBodyScript:GetEnvironment().turnBool
	...
	if ECS:hasComponent_Transform(EntID1) then
		...
		--Console.PrintFloat(bTransform.y)
	...
	boss["Boss_Env"] = ECS:getComponent_LuaScriptCom(bossHeadID)
	EntID1 = ScriptSystem:getLuaEntityID()
	...
	if bossHP < 75  then
		...
		if dissappearBool == false and canPlayLaserSound then
