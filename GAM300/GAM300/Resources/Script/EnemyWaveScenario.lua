...
function resetAll()
	...
	-- textPrefabID = ECS:getPrefabID("Prefab_Text")
end
	...
	if eEnemyTimer <= 0 then
		if spawnEnemyCount < eEnemyCountMax then
			...
			spawnPos.z = pos.z
					...
					ECS:addEntityTag(enemyID,"EnemyWaveScenario")
					ePrevXPos = eXPos
					ePrevZPos = eZPos
...
function updateEnemyCount()
	enemies = ECS:getEntityTagged("EnemyWaveScenario")
...
function Start()
	...
	-- fontKillCountID = ECS:instantiatePrefab(textPrefabID, "KILLCOUNTER", Vector3:new(0,0,0), eRot, false, false, false)
...
function Update(dt)
		...
		if eEntID == ECS:getEntityID("EnemyWaveScenario") then
			if ECS:hasComponent_Transform(eEntID) then
		 ...
		 if isTriggered == true then
			...
			if ECS:hasComponent_SpriteCom(UIObjID) then
				...
				spriteCom.SpriteName = "challenge_timelimit"
			end
			...
			if timer > 0 then
				timer = timer - dt
			elseif timer == 0 and restart == false then
				restart = true
				spawnEnemyCount = 0
				eEnemyCountMax = 5
			elseif timer < 0 then
			...
			if spawnEnemyCount == killObjective or restart == true then
			...
			if timer > 0 and killCount == killObjective and spawnEnemyCount > 0 then
				stopScript = true
			end
